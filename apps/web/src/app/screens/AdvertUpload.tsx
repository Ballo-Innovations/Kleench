      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="flex gap-3">
          {(["target", "general"] as AdvertType[]).map((t) => (
            ...
            >
              ...
              {t === "target" ? "Target Advert" : "General Advert"}
            </button>
          ))}
        </div>

        {/* Step indicator (target advert only) */}
        {advertType === "target" && (
          <div className="flex items-center gap-2">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                ...
                {s < 3 && <div className={`flex-1 h-0.5 w-8 rounded-full ${step > s ? "bg-orange-500" : "bg-slate-200"}`} />}
              </div>
            ))}
        ...
        <AnimatePresence mode="wait">
          {advertType === "target" && step === 1 && (
            <motion.div key="target-1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              <SectionCard>
                <div>
                  <FieldLabel>Content Type</FieldLabel>
                  <span className="inline-block bg-orange-500 text-white px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider">Advert</span>
                </div>

                <div>
                  <FieldLabel>Format</FieldLabel>
                  <TogglePills options={["Video", "Picture", "Audio"]} value={format} onChange={setFormat} />
                </div>
                ...
                {format === "Video" && (
                  <div>
                    ...
                    <TogglePills options={["15sec", "60sec", "10min"]} value={length} onChange={setLength} />
                  </div>
                )}
          ...
          {advertType === "target" && step === 2 && (
            <motion.div key="target-2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              <SectionCard>
                <div>
                  <FieldLabel>Gender</FieldLabel>
                  <TogglePills options={["All", "Male", "Female"]} value={gender} onChange={setGender} />
                </div>
                ...
                <div>
                  ...
                  <TogglePills options={["Single", "Married"]} value={prompt} onChange={setPrompt} />
                </div>
          ...
          {advertType === "general" && (
            <motion.div key="general" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              <SectionCard>
                ...
                <div>
                  <FieldLabel>Format</FieldLabel>
                  <TogglePills options={["Video", "Picture", "Audio"]} value={genFormat} onChange={setGenFormat} />
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(genFormat === "Video" ? SIZE_OPTIONS_VIDEO : SIZE_OPTIONS_AUDIO).map((s) => (
                      <button key={s} onClick={() => setGenSize(s)} className={`px-3 py-2 rounded-xl text-[11px] font-black transition-all active:scale-95 ${
                        genSize === s ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-500"
                      }`}>{s}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <button className="flex-1 h-11 bg-[var(--app-shape-accent)] text-white rounded-xl font-black text-[11px] uppercase tracking-wider active:scale-95 transition-all">
                    Calculate
                  </button>
