...
export function CreateInvestment() {
  ...
  const [bizRegUploaded, setBizRegUploaded] = useState(false);
  const [prospectusUploaded, setProspectusUploaded] = useState(false);
  const [finStmtUploaded, setFinStmtUploaded] = useState(false);
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={grace(0)} className="space-y-5">
              ...
              {[
                { label: "Business Registration *", uploaded: bizRegUploaded, set: setBizRegUploaded },
                { label: "Prospectus *", uploaded: prospectusUploaded, set: setProspectusUploaded },
                { label: "Financial Statement *", uploaded: finStmtUploaded, set: setFinStmtUploaded },
              ].map(({ label, uploaded, set }) => (
                <motion.button key={label} onClick={() => set(true)}
                  className={`w-full p-4 rounded-2xl border-2 flex items-center gap-4 transition-all active:scale-[0.98] ${uploaded ? 'border-green-400 bg-green-50' : 'border-dashed border-[var(--border)] bg-[var(--card)]'}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${uploaded ? 'bg-green-100' : 'bg-[var(--app-bg-muted)]'}`}>
                    {uploaded ? <Check size={20} className="text-green-500" strokeWidth={2.5} /> : <Upload size={20} className="text-[var(--color-secondary)]/40" strokeWidth={1.5} />}
                  </div>
                  <div className="text-left">
                    <p className={`font-black text-[13px] uppercase tracking-wide ${uploaded ? 'text-green-600' : 'text-[var(--color-secondary)]'}`}>
                      {uploaded ? `${label.replace(" *", "")} Uploaded` : `Upload ${label.replace(" *", "")}`}
                    </p>
                    <p className="text-[11px] font-semibold text-[var(--color-secondary)]/40">PDF or image — Max 10MB</p>
                  </div>
                </motion.button>
              ))}
