      ...
      <div className="px-5 pt-6 space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.1)} className="relative">
          ...
          <button onClick={() => setCatOpen(v => !v)}
            className="w-full px-4 py-3.5 rounded-2xl border border-[var(--border)] bg-white font-bold text-[14px] text-left flex items-center justify-between focus:border-[#E85D3F] transition-colors">
          ...
          {catOpen && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              className="absolute top-full left-0 right-0 z-20 mt-1 bg-white border border-[var(--border)] rounded-2xl shadow-lg overflow-hidden max-h-48 overflow-y-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.18)}>
          ...
          <button onClick={() => navigate("/kyc-verification")}
            className="w-full p-4 rounded-2xl border border-[var(--border)] bg-white flex items-center gap-3 active:scale-[0.98] transition-all">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.34)}>
          ...
          <button onClick={() => setLicenseUploaded(true)}
            className={`w-full p-4 rounded-2xl border-2 flex items-center gap-4 transition-all active:scale-[0.98] ${licenseUploaded ? 'border-green-400 bg-green-50' : 'border-dashed border-[var(--border)] bg-white'}`}>
