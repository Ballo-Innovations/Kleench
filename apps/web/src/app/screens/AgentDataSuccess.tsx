...
import { CheckCircle, Database, Plus, Home } from "lucide-react";
import { PageHeader } from "../components/PageHeader";

export function AgentDataSuccess() {
    ...
    <div className="w-full max-w-md mx-auto min-h-screen bg-transparent font-sans pb-32">
      ...
      <div className="px-5 pt-8 space-y-5">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200 }}
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#059669]">Data Received!</p>
          <p className="text-[22px] font-black text-[var(--app-text)] uppercase tracking-tight text-center leading-tight">
            {state?.commodity || "Commodity"}<br />Report Submitted
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="bg-[var(--color-secondary)] px-5 py-4">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/50">Submission ID</p>
            <p className="text-[18px] font-black text-white tracking-wider">{submissionId}</p>
          </div>
          <div className="px-5 py-4">
            {[
              { label: "Commodity", value: state?.commodity || "—" },
              { label: "Market", value: state?.market || "—" },
              { label: "Price", value: state?.price ? `K${state.price} ${state?.unit || ""}` : "—" },
              { label: "Availability", value: state?.availability || "—" },
              ...
              <div key={label} className="flex items-center justify-between py-2.5 border-b border-[var(--border)] last:border-0">
                ...
                <span className="text-[11px] font-bold text-[var(--color-secondary)] max-w-[60%] text-right">{value}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <p className="text-[11px] font-semibold text-[var(--color-secondary)]/70 leading-snug">
            Your submission will be verified and added to the KLeench Market Intelligence data. Thank you for contributing.
          </p>
        </motion.div>
      </div>
      ...
      <div className="px-5 pt-4 pb-8 space-y-3">
        <button onClick={() => navigate("/marketplace/agent/submit")}
          ...
          <Plus size={18} strokeWidth={2} /> Submit Another
        </button>
