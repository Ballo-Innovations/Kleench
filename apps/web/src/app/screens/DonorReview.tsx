...
const MOCK_PROFILE = {
  ...
  causes: ["Education", "Health", "Water & Sanitation"],
  frequency: "Monthly Donor",
};
...
export function DonorReview() {
  const navigate = useNavigate();
  const [submitting, setSubmitting] = useState(false);

  const handleSubmit = () => {
    setSubmitting(true);
    setTimeout(() => navigate("/donate/register-donor/success"), 1200);
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto min-h-screen font-sans pb-32">
      ...
      <div className="px-5 pt-6">
        ...
        <p className="text-[13px] font-semibold text-[var(--color-secondary)]/50 mb-6">Confirm your donor profile before submitting.</p>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.1)}
          className="bg-white rounded-2xl border border-[var(--border)] shadow-sm p-4 mb-6">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#E85D3F] mb-3">Donor Profile</p>
          ...
          <ReviewRow label="Causes" value={MOCK_PROFILE.causes} />
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.2)}
          ...
          <p className="text-[11px] font-bold text-[var(--color-secondary)]/60 leading-relaxed">
            By submitting, you agree to Kleench's donor terms and privacy policy. Your information is encrypted and securely stored.
          </p>
        </motion.div>
