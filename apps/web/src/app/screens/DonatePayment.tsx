    ...
    <div className="w-full max-w-md mx-auto min-h-screen font-sans pb-32">
      ...
      <div className="px-5 pt-5 space-y-5">
          ...
          <div className="grid grid-cols-5 gap-2">
            {PRESET_AMOUNTS.map(amt => (
              <button key={amt} onClick={() => { setSelected(amt); setCustom(""); }}
                className={`py-4 rounded-2xl font-black text-[12px] border-2 transition-all active:scale-95 ${selected === amt ? 'bg-[#E85D3F] border-[#E85D3F] text-white shadow-md' : 'bg-white border-[var(--border)] text-[var(--color-secondary)]'}`}>
                K{amt}
              </button>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.18)}>
          ...
          <div className="flex items-center px-4 py-3.5 rounded-2xl border-2 bg-white gap-2 transition-colors focus-within:border-[#E85D3F] border-[var(--border)]">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={grace(0.26)}
          className="bg-white rounded-2xl border border-[var(--border)] p-4 flex items-center gap-3 shadow-sm">
