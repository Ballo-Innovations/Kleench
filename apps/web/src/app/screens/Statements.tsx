    ...
    <div className="min-h-screen bg-transparent text-[#0D1B2A] font-sans pb-32">
      ...
      <div className="px-5 pt-4 space-y-8">
        ...
        <motion.div 
          ...
          className="bg-white rounded-3xl p-5 border-2 border-slate-100 shadow-[0_20px_50px_rgba(13,27,42,0.06)] space-y-5"
        >
            ...
            <div className="space-y-3 px-1">
              ...
              {SUMMARY_DATA.map((item, i) => (
                <div key={i} className="flex justify-between items-center">
                  ...
                  <span className="text-[12px] font-black text-[#0D1B2A]">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          ...
          <div className="overflow-x-auto -mx-1">
            <table className="w-full text-left">
              <thead>
                <tr className="text-[9px] font-black text-[#0D1B2A] uppercase tracking-tighter">
                  ...
                  <tr key={i} className="group">
                    <td className="py-4 pr-2 align-top">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-[#0D1B2A]">{row.date}</span>
                        <span className="text-[8px] font-bold text-[#6E7C91]">{row.time}</span>
                      </div>
                    </td>
                    <td className="py-4 pr-2 align-top min-w-[100px]">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-[#0D1B2A] leading-tight mb-0.5">{row.details}</span>
                        <span className="text-[8px] font-bold text-[#6E7C91] tracking-tighter truncate max-w-[80px]">({row.id})</span>
                      </div>
                    </td>
                    <td className="py-4 pr-1 text-right align-top text-[10px] font-black text-[#0D1B2A]">{row.credited}</td>
                    <td className="py-4 pr-1 text-right align-top text-[10px] font-black text-[#0D1B2A]">{row.debited}</td>
                    <td className="py-4 text-right align-top text-[10px] font-black text-[#0D1B2A]">{row.balance}</td>
                  </tr>
                ))}
              </tbody>
            </table>
